...
public class persistence_interval_front extends PersistenceModule {
    protected int observed_time_units;
    protected long set_panel_element;
    protected int observed_time_units_features;

    public persistence_interval_front(final int m, final int k, final int i, final int j) {
        super(m, k);
        observed_time_units = i;
        observed_time_units_features = j;
    }
            ...
            for (int i = 0; i < number_time_units; i++) {
                ...
                while (set_panel_iterator.hasNext()) {
                    ...
                    if (countpanel.containsKey(set_panel_element)) {
                        if ((i >= countpanel.get(set_panel_element) % (number_time_units + 1) + observed_time_units_features) && (i < countpanel.get(set_panel_element) % (number_time_units + 1) + observed_time_units + observed_time_units_features))
                            countpanel.addTo(set_panel_element, number_time_units + 1);
                    }
                    else {
                        if (i < number_time_units - observed_time_units - observed_time_units_features + 1)
                            countpanel.put(set_panel_element, i);
                    }
                }
            }
                ...
                for (int i = 0; i < number_time_units; i++) {
                    ...
                    while (set_panel_iterator2.hasNext()) {
                        ...
                        if (countpanel2.containsKey(set_panel_element2)) {
                            if ((i >= countpanel2.getInt(set_panel_element2) % (number_time_units + 1) + observed_time_units_features) && (i < countpanel2.getInt(set_panel_element2) % (number_time_units + 1) + observed_time_units + observed_time_units_features))
                                countpanel2.addTo(set_panel_element2, number_time_units + 1);
                        }
                        else {
                            if (i < number_time_units - observed_time_units - observed_time_units_features + 1)
                                countpanel2.put(set_panel_element2, i);
                        }
                    }
                }
